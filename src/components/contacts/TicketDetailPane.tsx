...
const TicketDetailPane = ({ ticketId, onBack }: TicketDetailPaneProps) => {
  ...
  const fetchTicket = useCallback(async () => {
    ...
    try {
      ...
      const [commentRes, aiRes] = await Promise.all([
        ...
        supabase.from("ticket_ai_analyses").select("*").eq("ticket_id", ticketId).maybeSingle(),
      ]);
  ...
  const handleAddComment = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from("ticket_comments").insert([
        { ticket_id: ticketId, user_id: session.user.id, content: newComment, is_internal: isInternal },
      ]);
      if (error) throw error;
      setNewComment("");
      fetchTicket();
  ...
  const handleLogTime = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("tickets").update({ actual_hours: newTotal }).eq("id", ticket.id);
      if (error) throw error;
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Client: {ticket.client_display_name}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="border-0 shadow-sm bg-ai-bg border-ai-border overflow-hidden animate-fade-in">
              ...
              <CardContent className="space-y-4">
                ...
                <p className="text-[10px] text-muted-foreground italic">
                  Last analyzed {format(new Date(aiAnalysis.updated_at), "MMM dd, h:mm a")}
                </p>
              </CardContent>
            </Card>
          )}
          ...
          <Card className="border-0 shadow-sm">
            <CardHeader className="pb-4 border-b">
              <CardTitle className="text-lg flex items-center gap-2">
                <Briefcase className="w-5 h-5 text-primary" /> Description
              </CardTitle>
            </CardHeader>
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-bold flex items-center gap-2 px-2">
              <MessageSquare className="w-5 h-5 text-primary" /> Conversation
            </h3>
            <div className="space-y-4">
              {comments.map((comment) => (
                <div
                  ...
                  className={cn(
                    ...
                    comment.is_internal ? "bg-warning-bg/50 border-warning-border" : "bg-background shadow-sm"
                  )}
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold">
                        {comment.user_id === session?.user.id ? "You" : "Client"}
                      </span>
                      {comment.is_internal && (
                        <Badge className="bg-warning-bg text-warning border-warning-border text-[9px] h-4 px-1.5">
                          Internal Note
                        </Badge>
                      )}
                    </div>
                ...
                <CardContent className="p-4 space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <Label htmlFor="is_internal" className="text-xs font-medium text-muted-foreground">
                        Internal Note
                      </Label>
                    </div>
            ...
            <CardContent className="p-6 space-y-6">
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline" className="rounded-lg px-3 py-0.5 text-xs font-semibold">
                    {ticket.category}
                  </Badge>
                </div>
                <div className="pt-4 border-t space-y-3">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground flex items-center gap-1.5">
                      <CheckCircle2 className="w-3.5 h-3.5" /> Actual Hours
                    </span>
                    <span
                      className={cn(
                        "font-bold",
                        ticket.actual_hours > (ticket.estimated_hours || 0) && ticket.estimated_hours > 0
                          ? "text-danger"
                          : "text-profit"
                      )}
                    >
          ...
          <Card className="border-0 shadow-xl bg-gradient-to-br from-violet-600 to-indigo-700 text-white overflow-hidden relative">
            ...
            <CardContent className="p-6 relative space-y-4">
              ...
              <p className="text-sm font-medium leading-relaxed">
                I can analyze this ticket to suggest a solution or summarize the conversation.
              </p>
              <Button
                variant="secondary"
      ...
      <Dialog open={showTimeDialog} onOpenChange={setShowTimeDialog}>
        <DialogContent className="sm:max-w-md rounded-2xl">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowTimeDialog(false)} className="rounded-xl">
              Cancel
            </Button>
            <Button onClick={handleLogTime} className="rounded-xl px-8" disabled={!timeToLog}>
              Log Time
            </Button>
          </DialogFooter>
        </DialogContent>
