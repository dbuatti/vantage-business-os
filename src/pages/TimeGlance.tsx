    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-8">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 animate-fade-in">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Focused view of your financial activity by period.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => navigate('today')} className="rounded-xl h-9 font-bold">
              Today
            </Button>
            <div className="flex items-center bg-muted rounded-xl p-1">
              <Button variant="ghost" size="icon" onClick={() => navigate('prev')} className="h-7 w-7 rounded-lg">
                <ChevronLeft className="w-4 h-4" />
              </Button>
              <Button variant="ghost" size="icon" onClick={() => navigate('next')} className="h-7 w-7 rounded-lg">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={stats.chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                      ...
                      <Tooltip
                        ...
                        contentStyle={{ backgroundColor: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: '12px', boxShadow: '0 8px 32px rgba(0,0,0,0.12)' }}
                      />
        ...
        <div className="space-y-8">
          ...
          {/* Quick Insights */}
          <Card className="border-0 shadow-xl bg-gradient-to-br from-primary to-indigo-700 text-white overflow-hidden relative animate-slide-up opacity-0 stagger-3">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(255,255,255,0.15),transparent_50%)]" />
            <CardContent className="p-6 relative space-y-4">
              <div className="flex items-center gap-2">
                <TrendingUp className="w-5 h-5" />
                <span className="text-xs font-black uppercase tracking-widest opacity-80">Period Insight</span>
              </div>
              <div className="space-y-2">
                <p className="text-sm font-medium leading-relaxed">
                  {stats.net >= 0 
                    ? `You're currently up ${formatCurrency(stats.net)} for this ${view}. Great job maintaining a positive cash flow!`
                    : `You've spent ${formatCurrency(Math.abs(stats.net))} more than you've earned this ${view}. Consider reviewing your top categories.`}
                </p>
                {stats.pieData.length > 0 && (
                  <p className="text-xs opacity-70">
                    Your largest expense category is <span className="font-bold">{stats.pieData[0].name}</span>.
                  </p>
                )}
              </div>
              <Button variant="secondary" className="w-full rounded-xl font-bold gap-2 shadow-lg" asChild>
                <Link to="/insights">Full AI Analysis <ArrowRight className="w-4 h-4" /></Link>
              </Button>
            </CardContent>
          </Card>
        </div>
